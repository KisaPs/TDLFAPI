...
### Адрес для отправки остатков спецзаказов в Заказах покупателя в формате json
```http
POST PostStockSpecIFZ
```
...
```diff
{
  "ОстаткиСпецТоваровИФЗ": [
    {
      "УникальныйИдентификаторЗаказа": "0a276748-5f58-11ed-bd7a-d4f5ef7885bf",
      "НомерЗаказаИФЗ": "12345",
      "ДатаДокумента": "01.01.2025 20:01:03",
      "Комментарий" : "Старые цены"
      "СписокТоваров": [
        {
          "НаименованиеТовара": "ЧАШ.С БЛ.Чер.кофеТонкие веточ.с отв.КН1",
          "АртикулТовара": "81.13635.00.1",
...
<details>
<summary><b> Пример положительного ответа сервиса на запрос списка заказов (PostStockSpecIFZ):</b> (нажмите, чтобы развернуть)</summary>
